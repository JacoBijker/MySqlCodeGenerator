...
{
    public static class IDalGenerator
    {
        public static void Run(string path, string nspace, string tableName, List<Column> columns)
        {
            ...
            var filename = $"I{tableName}Repository.Gen.cs";
            fileString.SaveTo($"{path}\\Dal\\{nspace}.Dal.Interface\\Gen\\{filename}");
        }
        ...
        private static string GenerateFileString(string nspace, string tableName, List<Column> columns)
        {
            ...
            sb.Append($"namespace {nspace}.Dal.Interface.Gen{Environment.NewLine}");
            sb.Append($"{{{Environment.NewLine}");
            sb.Append($"    public partial interface I{tableName}Repository{Environment.NewLine}");
            sb.Append($"    {{{Environment.NewLine}");
